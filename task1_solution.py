...
out_channels = custom_grouped_layer.out_channels

# copying weights and bias from the original 2d convolutin
for i in range(groups):
    
    start_idx = i*(out_channels//groups)
    end_idx = (i+1)*(out_channels//groups)
    
    custom_grouped_layer.convs[i].weight.data = grouped_layer.weight.data[start_idx:end_idx, :, :, :]
    custom_grouped_layer.convs[i].bias.data = grouped_layer.bias.data[start_idx:end_idx]
...
''' For the CustomGroupedConv2D layer the in_channels and out_channels should be divisible by groups'''